...
class BitsharesStatisticsParser(BlockchainTransactionsParser):
    ...
    def analyse_orders_on_blockchain(self):
                        ...
                        for operation in transaction["operations"]:
                            if operation[0] != 0:
                                ...
                                if operation[0] == 1:  # If we have a new order...
                                    signature_order_map[transaction["signatures"][cur_ind]] = \
                                        transaction["operation_results"][cur_ind][1]
        ...
        # Parse the account histories of users and create a map that stores the order_id
        # with the block that captures the order fulfillment.
        for _, filename, _ in self.yield_files('history.txt'):
        ...
        avg_latency = 0
        avg_latency_count = 0
        for _, filename, _ in self.yield_files('created_orders.txt'):
    ...
    def parse_transactions(self):
        ...
        # We go over all transactions created by client.
        # Check if the signature is included in the blockchain and compute the transaction latency.
        for peer_nr, filename, _ in self.yield_files('tx_submit_times.txt'):
