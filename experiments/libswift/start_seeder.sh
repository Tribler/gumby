...
sudo /usr/bin/lxc-execute -n seeder \
	...
	-s lxc.pts=1024 \
	-- $WORKSPACE_DIR/$SEEDER_CMD $WORKSPACE_DIR/swift $OUTPUT_DIR $FILENAME $SEEDER_DELAY $SEEDER_PACKET_LOSS $WORKSPACE_DIR/gumby/scripts/process_guard.py $EXPERIMENT_TIME $BRIDGE_IP $SEEDER_PORT $OUTPUT_DIR $USER $SEEDER_RATE $SEEDER_RATE_UL $IPERF_TEST &
