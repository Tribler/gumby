...
sudo /usr/bin/lxc-execute -n leecher_$i \
		...
		-s lxc.pts=1024 \
		-- $WORKSPACE_DIR/$LEECHER_CMD $REPOSITORY_DIR $OUTPUT_DIR $HASH $NETEM_DELAY $NETEM_PACKET_LOSS $WORKSPACE_DIR/$PROCESS_GUARD_CMD $EXPERIMENT_TIME $BRIDGE_IP $SEEDER_IP $SEEDER_PORT $OUTPUT_DIR $(($LEECHER_ID+$i)) &
	#LEECHER_PIDS[$i]=$!
done
