...
if(file.exists("threads.txt")){
	df <- read.table("threads.txt", header = TRUE, check.names = FALSE, na.strings = "?")
	num_columns <- ncol(df) - 1
	df <- mean_max_min(num_columns, df)
	df$type <- 'Process'

	if(file.exists("threads_node.txt")){
		df2 <- read.table("threads_node.txt", header = TRUE, check.names = FALSE, na.strings = "?")
	...
	p <- p + labs(x = "\nTime into experiment (Seconds)", y = "Number of threads\n")
	if(length(args) > 0){
		p <- p + xlim(minX, maxX)
	}
	p

	ggsave(file="threads.png", width=12, height=6, dpi=100)
}