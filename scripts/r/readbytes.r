...
if(file.exists("rchars_reduced.txt")){
	df <- read.table("rchars_reduced.txt", header = TRUE, check.names = FALSE, na.strings = "?")
	num_columns <- ncol(df) - 1
	df <- mean_max_min(num_columns, df)
	df$type <- 'Process'

	if(file.exists("rchars_node_reduced.txt")){
		df2 <- read.table("rchars_node_reduced.txt", header = TRUE, check.names = FALSE, na.strings = "?")
