...
class DispersyModule(BaseDispersyModule):
    ...
    def set_dispersy_crypto(self, value):
        if value == 'NoCrypto':
            ...
            self._logger.error('set_disperys_crypto argument invalid, defaulting to ECCrypto')
            self.crypto = ECCrypto()

    @inlineCallbacks
    def _do_log(self):
        if not hasattr(self, "_do_log_env") or not self._do_log_env:
            self._do_log_env = object()
            self._do_log_env.total_stumbled_candidates = defaultdict(lambda: defaultdict(set)),
            self._do_log_env.prev_statistics = {},
            self._do_log_env.prev_total_received = {},
            self._do_log_env.prev_total_dropped = {},
            self._do_log_env.prev_total_delayed = {},
            self._do_log_env.prev_total_outgoing = {},
            self._do_log_env.prev_total_fail = {},
            self._do_log_env.prev_endpoint_recv = {},
            self._do_log_env.prev_endpoint_send = {},
            self._do_log_env.prev_created_messages = {}
